...
<script>
  export default {
    ...
    watch: {
      row (val) {
        this.loadImg = process.env.NODE_ENV === 'production' ? process.env.API + 'files' + val.posterUrl.split('files')[1] : 'https://www.juniwota.com/gather' + val.posterUrl
      }
    },
    methods: {
      clickItem () {
    ...
    beforeCreate () {
      this.$nextTick(() => {
        if(this.row.posterUrl){
          this.loadImg = process.env.NODE_ENV === 'production' ? process.env.API + 'files' + this.row.posterUrl.split('files')[1] : 'https://www.juniwota.com/gather' + this.row.posterUrl
        }
      })
    }
  }
